...
class Guardian extends Base {
    ...
    public static function get($key = null, $fallback = "") {
        if($log = Session::get('cookie:' . self::$user, false)) {
            foreach(Filter::apply('user', $log, $log['user']) as $k => &$v) {
                $v = Filter::apply('user:' . $k, $v, $log);
            }
            unset($v);
            if( ! is_null($key)) {
        ...
        if($file = File::exist(LOG . DS . 'users.txt')) {
            ...
            foreach(explode("\n", file_get_contents($file)) as $str) {
                ...
                // Pattern 2: `user: pass (Author Name @status) email@domain.com`
                    ...
                    $str['user'] = $matches[1];
                    $str['pass'] = $matches[2];
                    $str['author'] = $matches[3];
                    $str['status'] = $matches[4];
                    $str['email'] = isset($matches[5]) && ! empty($matches[5]) ? $matches[5] : false;
                    $user = $matches[1];
                }
                $str['username'] = $str['user']; // alias for `user`
                $str['password'] = $str['pass']; // alias for `pass`
                if(self::happy() && Session::get('cookie:' . self::$user . '.status') !== 1) {
                    $str['password'] = $str['pass'] = null;
                }
                foreach($str as $k => &$v) {
                    $v = Filter::apply('user:' . $k, $v, $str);
                }
