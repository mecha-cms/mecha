...
<form class="form-repair form-comment" action="<?php echo $config->url_current; ?>" method="post">
  <?php $wb = Guardian::wayback(); ?>
  <input name="token" type="hidden" value="<?php echo Guardian::makeToken(); ?>">
  <label class="grid-group">
    <span class="grid span-1 form-label"><?php echo $speak->comment_name; ?></span>
    <span class="grid span-5"><input name="name" type="text" class="input-block" value="<?php echo $wb['name']; ?>"></span>
  </label>
  <label class="grid-group">
    <span class="grid span-1 form-label"><?php echo $speak->comment_email; ?></span>
    <span class="grid span-5"><input name="email" type="text" class="input-block" value="<?php echo $wb['email']; ?>"></span>
  </label>
  <label class="grid-group">
    <span class="grid span-1 form-label"><?php echo $speak->comment_url; ?></span>
    <span class="grid span-5"><input name="url" type="text" class="input-block" value="<?php echo $wb['url']; ?>"></span>
  </label>
  <label class="grid-group">
    ...
    <span class="grid span-5">
      ...
      foreach(array('pilot' => $speak->pilot, 'passenger' => $speak->passenger, 'intruder' => $speak->intruder) as $key => $value) {
          echo '<option value="' . $key . '"' . ($wb['status'] == $key ? ' selected' : "") . '>' . $value . '</option>';
      }
