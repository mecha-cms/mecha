...
<form class="form-kill form-page" action="<?php echo $config->url_current; ?>" method="post">
  ...
  <?php endif; ?>
  <p><button class="btn btn-primary btn-delete" type="submit"><i class="fa fa-check-circle"></i> <?php echo $speak->yes; ?></button> <a class="btn btn-danger btn-cancel" href="<?php echo $config->url . '/' . $config->manager->slug . '/' . $config->editor_type . '/repair/id:' . $page->id; ?>"><i class="fa fa-times-circle"></i> <?php echo $speak->no; ?></a></p>
</form>