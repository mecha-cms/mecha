...
Route::post($config->manager->slug . '/ajax/preview:(article|page)', function($kind = "") use($config, $speak) {
    ...
    ))->content(Request::post('content', Config::speak('notify_empty', array(strtolower($speak->contents)))))->read();
...
Route::post($config->manager->slug . '/ajax/preview:comment', function() use($config, $speak) {
    ...
    $comment = Page::header('Content Type', Request::post('content_type', 'HTML'))->content(Request::post('message', Config::speak('notify_empty', array(strtolower($speak->contents)))))->read('message', 'comment:');
    Weapon::fire('preview_content_before', array($P, $P));
