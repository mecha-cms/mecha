...
Route::accept(array($config->manager->slug . '/page/ignite', $config->manager->slug . '/page/repair/id:(:num)'), function($id = false) use($config, $speak) {
    if($id && $page = Get::page($id, array('content', 'excerpt', 'tags'))) {
        ...
        if( ! Guardian::happy(1) && Guardian::get('author') !== $page->author) {
            Shield::abort();
        }
    ...
    if($request = Request::post()) {
        ...
        if( ! Notify::errors()) {
            ...
            if( ! $id) {
                ...
                Notify::success(Config::speak('notify_success_created', $title) . ($extension === '.txt' ? ' <a class="pull-right" href="' . Filter::colon('page:url', $config->url . '/' . $slug) . '" target="_blank"><i class="fa fa-eye"></i> ' . $speak->view . '</a>' : ""));
                Weapon::fire(array('on_page_update', 'on_page_construct'), array($G, $P));
            ...
            // Repair
                ...
                Notify::success(Config::speak('notify_success_updated', $title) . ($extension === '.txt' ? ' <a class="pull-right" href="' . Filter::colon('page:url', $config->url . '/' . $slug) . '" target="_blank"><i class="fa fa-eye"></i> ' . $speak->view . '</a>' : ""));
                Weapon::fire(array('on_page_update', 'on_page_repair'), array($G, $P));
                Guardian::kick($config->manager->slug . '/page/repair/id:' . Date::format($date, 'U'));
            }
        }
    }
...
Route::accept($config->manager->slug . '/page/kill/id:(:num)', function($id = "") use($config, $speak) {
    ...
    if($request = Request::post()) {
        ...
        Weapon::fire(array('on_page_update', 'on_page_destruct'), array($G, $G));
