...
<form class="form-<?php echo $path === false ? 'ignite' : 'repair'; ?> form-shield" id="form-<?php echo $path === false ? 'ignite' : 'repair'; ?>" action="<?php echo $config->url_current . $config->url_query; ?>" method="post">
  ...
  <p>
    <?php echo Form::text('name', Request::get('name', Guardian::wayback('name', File::url($path))), $speak->manager->placeholder_file_name); ?>
    ...
    <?php echo Jot::btn('destruct', $speak->delete, $config->manager->slug . '/shield/' . $folder . '/kill/key:' . $id); ?>
    <?php endif; ?>
  </p>
</form>